...
# -j y -R y -pe smp 1 -V /share/ClusterShare/thingamajigs/jamtor/projects/hgsoc_repeats/RNA-seq/scripts/rsem_no_mmappers/6.transfer_gc_to_massdata.py
...
for f in files:
	...
	if os.path.isfile(gc_dir + '/' + f + '/Aligned.toTranscriptome.out.bam'):
		...
		if os.path.isfile(gc_dir + '/' + f + '.tar.gz'):
			...
			os.system('rsync -avPS ' + gc_dir + '/no_mmappers/' + f + '.tar.gz ' + nci_username + \
				':' + nci_gc)
			os.system(script_dir + '/7.transfer_gc_to_massdata.bash')
