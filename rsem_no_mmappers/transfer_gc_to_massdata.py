...
# qsub line:
# run 'source activate p3env' before submitting the following:
# qsub -q short.q -N gctr -b y -wd \
#/share/ClusterShare/thingamajigs/jamtor/projects/hgsoc_repeats/RNA-seq/logs/rsem_no_mmappers \
#-j y -R y -pe smp 1 -V python \
#/share/ClusterShare/thingamajigs/jamtor/projects/hgsoc_repeats/RNA-seq/scripts/rsem_no_mmappers/transfer_gc_to_massdata.py
import os
import glob
import getpass
import pexpect
...
script_dir = home_dir2 + \
	'/projects/hgsoc_repeats/RNA-seq/scripts/' + exp_name + \
	 '/'
...
for f in files:
	if os.path.isfile(gc_dir + '/' + f + '/Log.final.out'):
		print('')
		...
		if os.path.isfile(gc_dir + '/' + f + '/Aligned.toTranscriptome.out.bam'):
			print('')
			print('Tarring ' + f)
