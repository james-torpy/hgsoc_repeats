...
STARgcDir <- paste0(projectDir, "/results/star/GC/", expName, "/logs/")
...
#combined[duplicated(combined)] <- gsub("AOCS", "AOCS2", combined[duplicated(combined)])
#combined[duplicated(combined)] <- gsub("AOCS2", "AOCS3", combined[duplicated(combined)])
...
sampleNames <- as.character(read.table(paste0(htseqDir, "/completed_files.txt"))[,1])
