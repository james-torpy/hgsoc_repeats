...
for (Counts in countDFs) {
  ...
  for (df in list(CPM, CPMR)) {
    ...
    plot_itdf <- function(x, logNo="nope") {
      if (ncol(x) == 3) {
        p <- ggplot(x, aes(x=sample, y=pDF[,3], group=repeat_id, colour=repeat_id))
        p <- p + geom_line()
        ...
        p <- ggplot(x, aes(x=sample, y=pDF[,3], group = 1))
        p <- p + geom_line()
        p <- p + theme(axis.text.x = element_text(angle = 90), legend.position = "bottom")
        if (logNo=="log10") {
          p <- p + scale_y_log10()
      ...
      #} else {
        print(paste0("Creating ", plotDir, "/", Type, "_", names(countDFs)[j], typeNames[n], ".pdf"))
        pdf(file = paste0(plotDir, "/", Type, "_", names(countDFs)[j], typeNames[n], ".pdf"), width = 10, height=10)
        print(pdfPlot)
        dev.off()
      #}
      ...
      #} else {
        print(paste0("Creating ", plotDir, "/", Type, "_", names(countDFs)[j], typeNames[n], "Log10.pdf"))
        pdf(file = paste0(plotDir, "/", Type, "_", names(countDFs)[j], typeNames[n], "Log10.pdf"), width = 10, height=10)
        print(pdfPlotLog10)
        dev.off()
      #}
    n=n+1
