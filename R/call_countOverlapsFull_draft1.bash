...
outScript="$scriptDir/countOverlapsFull.R"
...
i=0
directory_names=`ls $bamPath | grep -v subset | grep -v ribosome | grep grant_endosis1`
...
while [ $j -lt ${#directoriesTotal[@]} ]; do
	...
	qsub -q short.q -b y -j y -N tcount$uID -wd $logDir -pe smp 1 -l mem_requested=40G -V $RDir $submitLine
