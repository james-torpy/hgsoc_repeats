...
Counts <- lapply(Genes, function(x) {
  return(count_it(x))
})
CountsDF <- as.data.frame(do.call("rbind", Counts))

# create output directory:
outDir <- paste0(RobjectDir, "/", annot, "_RepeatCountDFs/")
system("mkdir -p outDir")

# save the counts as RDS files:
if (file.exists(paste0(outDir, "/", uID, "_", annot, "RepeatCountsDF.rds already exists, no need to create"))) {
  print(paste0(outDir, "/", uID, "_", annot, "RepeatCountsDF.rds"))
} else {
  print(paste0("Creating", outDir, "/", uID, "_", annot, "RepeatCountsDF.rds"))
  saveRDS(CountsDF, file = paste0(outDir, "/", uID, "_", annot, "RepeatCountsDF.rds"))
}
