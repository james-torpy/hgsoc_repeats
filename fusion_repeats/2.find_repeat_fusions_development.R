...
#-j y -R y -pe smp 4 -V Rscript /share/ClusterShare/thingamajigs/jamtor/projects/hgsoc_repeats/RNA-seq/scripts/fusion_repeats/2.find_repeat_fusions_development.R
...
RobjectDir <- paste0(projectDir, "/RNA-seq/Robjects/",
                     expName, "/", descrip, "/develop/")
plotDir <- paste0(resultsDir, "/R/", expName,
                  "/plots/", descrip, "/develop")
...
######
inFiles <- inFiles[c(1, 12, 23, 38, 46, 80)]
######

print(paste0("The inFiles are:", inFiles))

# define sample IDs:
sID <- strsplit(inFiles, "/")
sID <- unlist(
...
if ( upTo < length(inFiles) ) {
  ...
  writeLines("\n")
  print(paste0("Loading in cRanges..."))
  cRanges <- readRDS(paste0(RobjectDir, "/cRanges.rds"))
}


if ( !file.exists(paste0(RobjectDir, "/allAnnot.rds")) ) {
  writeLines("\n")
  print(paste0("Loading in repeat annotation..."))
  repeatAnnot <- import(paste0(rpAnnotDir, "/human-89.repeats.gtf"))
  
  # change 'type' column name to 'gene_id':
  colnames(values(repeatAnnot))[colnames(values(repeatAnnot)) == 'type'] <- 'gene_id'
  ...
  saveRDS(allAnnot, paste0(RobjectDir, "/allAnnot.rds"))

} else {

  writeLines("\n")
  print(paste0("Loading in allAnnot..."))
  allAnnot <- readRDS(paste0(RobjectDir, "/allAnnot.rds"))
  
  repeatAnnot <- allAnnot[allAnnot$source == "repbase"]
  gcAnnot <- allAnnot[allAnnot$source != "repbase"]
...
if ( !file.exists(paste0(RobjectDir, "/rOverlaps.rds")) ) {

  # find overlaps between chimera entries and the annotation ranges for
  # each sample:
  writeLines("\n")
  ...
  # name the sample overlaps:
  names(rOverlaps) <- sID  
  saveRDS(rOverlaps, paste0(RobjectDir, "/rOverlaps.rds"))

} else {

  writeLines("\n")
  print(paste0("Loading in rOverlaps..."))
  rOverlaps <- readRDS(paste0(RobjectDir, "/rOverlaps.rds"))

}

# check if rChims has been partially/fully created before and if so,
# load in to prevent repeating steps:
...
if ( upTo < length(rOverlaps)) {
  for (j in upTo:length(rOverlaps)) {
    
    print(j)
    
    # create repeatHits entry to add hits to cRanges elements:
    cRanges[[j]]$repeatHits <- rep(NA, length(cRanges[[j]]))
    
    # add repeat fusion hits to cRanges elements, recording 
    # each fusion once with the total number of reads supporting
    # it:
    for (i in 1:length(queryHits(rOverlaps[[j]]))) {

      # define query and subject hits:
      qHit <- queryHits(rOverlaps[[j]])[i]
      sHit <- subjectHits(rOverlaps[[j]])[i]
      print(qHit)
      
      # make a record of each type of query hit:
      if (i==1) {
        record <- c(qHit)
      } else {
        record[i] <- qHit
      ...
      # keep a record of duplicate query hits:
      dup <- table(record)
      dupNo <- dup[names(dup)==qHit]
      
      if (dupNo==1) {
        cRanges[[j]]$repeatHits[qHit] <- as.character(repeatAnnot[sHit]$gene_id)
      } else {
        cRanges[[j]][qHit]$repeatHits <- paste0(cRanges[[j]][qHit]$repeatHits, ",", as.character(repeatAnnot[sHit]$gene_id))
      }
    }
    
    # create repeat_chimeras list:
    if (j==1) {
      repeat_chimeras <- list(cRanges[[j]][!is.na(cRanges[[j]]$repeatHits)])
    } else {
      repeat_chimeras[[j]] <- cRanges[[j]][!is.na(cRanges[[j]]$repeatHits)]
    }
    
    saveRDS(paste0(RobjectDir, "/repeat_chimeras.rds"))
    ...
    for (n in 1:length(queryHits(acceptorOverlaps))) {
      qHit <- queryHits(acceptorOverlaps)[n]
      sHit <- subjectHits(acceptorOverlaps)[n]
      print(qHit)
