...
#align reads of input files with STAR, output into .bam files:
	starJobName="star."$uniqueID
       ...
       outSortedBam=$outDir"$uniqueID.sorted.bam"
     	star_line="STAR --genomeDir $genomeDir --runMode alignReads --readFilesCommand zcat --readFilesIn $inFile1 $inFile2 --outFileNamePrefix $outDir --runThreadN $numcores --outSAMattributes Standard --sjdbOverhang 99 --outReadsUnmapped Fastx"
       echo $star_line

#submit jobs to the cluster, creating a log in $logDir which includes reported errors:                	
  qsub -N nenrSTAR_$uniqueID -hold_jid TRIMGALORE_$UniqueID -b y -wd $logDir -j y -R y -pe smp $numcores -V $star_line
