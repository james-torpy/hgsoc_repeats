...
outPath="$resultsDir/$outType/$methodName/ribosome"
...
while [ $j -lt ${#filesTotal[@]} ]; do
  ...
  #| sed 's/_val_1$inExt//'`
  outDir="$outPath/$uniqueID/basic_1mmap_mapLength$mapL"
	...
	starJobName="star."$uniqueID
       ...
       outSortedBam=$outDir"$uniqueID.sorted.bam"
     	star_line="STAR --runMode alignReads \
      	...
      	--genomeDir $genomeDir \
      --outFilterMatchNminOverLread $mapL \
      --outFilterScoreMinOverLread $mapL \
...
#submit jobs to the cluster, creating a log in $logDir which includes reported errors:                	
  qsub -N rSTAR_$uniqueID -hold_jid TRIMGALORE_$UniqueID -b y -wd $logDir -j y -R y -pe smp $numcores -V $star_line
